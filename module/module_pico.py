...
def speak(content):
    ...
    #subprocess.call('amixer sset Master 90% -q --quiet', shell=True)  # loud
    subprocess.call(['pico2wave', '-w={}'.format(speech_wave), content])
    subprocess.call('aplay -q --quiet {}'.format(speech_wave), shell=True)
    #subprocess.call('amixer sset Master 75% -q --quiet', shell=True)  # restore the loudness of the voice
