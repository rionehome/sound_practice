...
csv_file = input('辞書にしたいtxtファイル名を入力してください(.txtは含まず)：')  # csvファイルを入力
txt_file = input('加えたいノイズ１欄のtxtファイル名を入力してください(.txtは含まず)：')  # txtファイルを入力

file_path = os.path.abspath(__file__)

# Define path
PATH0 = file_path.replace('tools/gram_maker_from_txt.py', 'dictionary/QandA/{}.txt').format(
    csv_file)  # 用意したq&aファイルの絶対パス
PATH1 = file_path.replace('tools/gram_maker_from_txt.py', 'dictionary/cmudict-en-us.dict')  # pocketsphinxの元の辞書の絶対パス
PATH2 = file_path.replace('tools/gram_maker_from_txt.py', 'dictionary/{}.dict').format(name)  # pocketsphinxの作りたい辞書の絶対パス
PATH3 = file_path.replace('tools/gram_maker_from_txt.py', 'dictionary/{}.gram').format(
    name)  # pocketsphinxの作りたい文法辞書の絶対パス
PATH4 = file_path.replace('tools/gram_maker_from_txt.py', 'dictionary/noise/{}.txt').format(
    txt_file)  # 用意したノイズファイルの絶対パス
...
if os.path.exists(PATH2):
    ...
    with open(PATH0, "r") as f:
        ...
        for qa in read_line:
            sentence_list.append(str.lower(qa.rstrip().split(",")[0]))
